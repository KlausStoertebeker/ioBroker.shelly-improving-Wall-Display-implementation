...
/**
 * Adds a generic thermostat definition for Gen 2+ devices
 *
 * This function defines the necessary states and MQTT commands for controlling a thermostat
 * device using the Shelly Gen 2 platform.
 * It allows for enabling/disabling the thermostat, adjusting the thermostats target temperature,
 * turning the output on/off, reading the current temperature.
 * Schedules may be added later.
 *
 * @param {object} deviceObj - The device object where states and commands are added.
 * @param {number} thermostatId - The ID of the thermostat (used in MQTT topics and commands).
 *
 * The following states are added:
 * - enable thermostat (enabling/disabling the thermostat functionality)
 * - reading/writing target temperature (thermostat target temperature from 5 to 35 degree Celsius in 0.5 steps)
 * - reading current temperature
 * - switching output on and off
 * - Channel Name (custom name for the channel)
 * - Source (source of the last command issued to the thermostat)
 * - Event (event notifications related to the thermostat)
 */

function addThermostat(deviceObj, thermostatId) {
    deviceObj[`thermostat${thermostatId}.Enable`] = {
        mqtt: {
            mqtt_publish: `<mqttprefix>/status/thermostat:${thermostatId}`,
            mqtt_publish_funct: value => JSON.parse(value).enable,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
                    src: 'iobroker',
                    method: 'enable.Set',
                    params: { id: thermostatId, enable: value },
                });
            },
        },
        common: {
            name: {
                en: 'enable thermostat',
                de: 'Thermostat aktivieren',
                ru: '',
                pt: '',
                nl: '',
                fr: '',
    ...
    deviceObj[`thermostat${thermostatId}.Target_C`] = {
        mqtt: {
            mqtt_publish: `<mqttprefix>/status/thermostat:${thermostatId}`,
            mqtt_publish_funct: value => JSON.parse(value).target_C,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
                    src: 'iobroker',
                    method: 'target_C.Set',
                    params: { id: thermostatId, target_C: value },
                });
            },
        },
        common: {
            name: {
                en: 'Set point temperature',
                de: 'SOLL-Temperatur',
                ru: '',
                pt: '',
                nl: '',
...
deviceObj[`thermostat${thermostatId}.Current_C`] = {
        mqtt: {
            mqtt_publish: `<mqttprefix>/status/thermostat:${thermostatId}`,
            mqtt_publish_funct: value => JSON.parse(value).current_C,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
                    src: 'iobroker',
                    method: 'current_C.Get',
                    params: { id: thermostatId, current_C: value },
                });
            },
        },
        common: {
        ...
        deviceObj[`thermostat${thermostatId}.Output`] = {
        mqtt: {
            mqtt_publish: `<mqttprefix>/status/thermostat:${thermostatId}`,
            mqtt_publish_funct: value => JSON.parse(value).output,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    ...
                    src: 'iobroker',
                    method: 'output.Set',
                    params: { id: thermostatId, ouput: value },
                });
            },
        },
        common: {
    ...
    deviceObj[`thermostat${thermostatId}.ChannelName`] = {
        mqtt: {
            http_publish: `/rpc/thermostat.GetConfig?id=${thermostatId}`,
            http_publish_funct: async (value, self) => {
                return value ? await shellyHelper.setChannelName(self, `thermostat${thermostatId}`, JSON.parse(value).name) : undefined;
            },
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    ...
                    params: { id: thermostatId, config: { name: value } },
                });
            },
        },
        common: {
            ...
            read: true,
            write: true,
            def: `thermostat_${thermostatId}`,
        },
    };

    deviceObj[`thermostat${thermostatId}.source`] = {
        mqtt: {
            mqtt_publish: `<mqttprefix>/status/thermostat:${thermostatId}`,
            mqtt_publish_funct: value => JSON.parse(value).source,
        },
    ...
    deviceObj[`thermostat${thermostatId}.Event`] = {
        mqtt: {
            mqtt_publish: '<mqttprefix>/events/rpc',
            mqtt_publish_funct: value => {
                const valueObj = JSON.parse(value);
                if (valueObj?.method === 'NotifyEvent' && valueObj?.params?.events) {
                    for (const e in valueObj.params.events) {
                        const event = valueObj.params.events[e];
                        if (typeof event === 'object' && event.component === `thermostat:${thermostatId}`) {
                            return event.event;
                        }
                    }
                }
